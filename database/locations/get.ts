...
// Lấy tất cả các bản ghi trong bảng `locations`
const getAllLocations = async (params: QueryParams = {}) => {
    const { columns = '*', filters = {} } = params;

    let query = supabase.from('locations').select(columns);
...
// Lấy bản ghi trong `locations` với các cột cụ thể và bảng tham chiếu
const getSpecificLocations = async (params: QueryParams = {}) => {
    const {
...
// Lấy `locations` với điều kiện lọc phức tạp
const getLocationsWithFilter = async (params: QueryParams = {}) => {
...
export { getAllLocations, getSpecificLocations, getLocationsWithFilter };
