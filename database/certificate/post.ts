...
type CertificateData = Record<string, any>; // Dữ liệu truyền vào cho insert/upsert/update

// Hàm chèn một bản ghi vào bảng `certificates`
const insertCertificate = async (data: CertificateData) => {
...
// Hàm chèn nhiều bản ghi vào bảng `certificates`
const insertManyCertificates = async (dataArray: CertificateData[]) => {
    const { data: insertedData, error } = await supabase
        .from('certificates')
...
const upsertCertificates = async (dataArray: CertificateData[]) => {
...
const updateCertificate = async (data: CertificateData, filters: Record<string, any>) => {
    let query = supabase
        .from('certificates')
...
export { insertCertificate, insertManyCertificates, upsertCertificates, updateCertificate };
