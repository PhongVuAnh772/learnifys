...
notifyServer.on("connection", (socket) => {
  ...
  notifyServer.emit("get-list-user-online", {
    listUserOnline: arrSocketsNotifyServer,
  });
  ...
  socket.on("send-request-borrow", async ({ senderId }) => {
    arrSocketsNotifyServer.map(async (item) => {
      if (item.roleId === "R1") {
        await axios.post(
          `${process.env.URL_BACK_END}/api/auth/add-one-notifycation-socket`,
          {
            titleId: "ST",
  ...
  socket.on(
    ...
    async ({
      ...
      if (roomId) {
        ...
        let receiverUser = arrSocketsNotifyServer.find(
