...
class userController {
  getListAssignmentsOfClassByClassId = async (req, res) => {
    try {
      ...
      let data = await examService.getListAssignmentsClassByClassId({
        classId,
      });
      let listExamId = data.map((item) => item.id);
      let pointData = await resultHistoryService.getPointExamByListExamId({
        listExamId,
      });
  ...
  getListDocumentsOfClassByClassId = async (req, res) => {
    try {
      ...
      let data = await documentService.getListDocumentsClassByClassId({
        classId,
      });
  ...
  getListQuestionsOfExamByExamId = async (req, res) => {
    try {
      ...
      let data = await examQuestionService.getListQuestionOfExamByExamId({
        examId,
      });
  ...
  getContentAssignmentOfClassById = async (req, res) => {
    try {
      ...
      let dataQuestion =
        await examQuestionService.getListQuestionOfExamForGroup({ examId });
  ...
  getListClass = async (req, res) => {
    try {
      let userId = req.userId;
      let data = await studentClassService.getListClassByStudentId({
        studentId: userId,
      });
  ...
  searchStudent = async (req, res) => {
    try {
      ...
      if (roleId === keyMap.roleId.PARENTS) {
        listUserIdSkip = await relationshipService.getListStudentIdOfParents({
          parentsId: userId,
        });
      } else {
        listUserIdSkip = await studentClassService.getListStudentIdOfClass({
          classId,
        });
      }
      let data = await userService.searchStudent({
        search,
        listUserIdSkip: listUserIdSkip,
      });
  ...
  searchTeacher = async (req, res) => {
    try {
      ...
      let data = await userService.searchTeacher({
        search,
        userIdSkip: userId,
      });
  ...
  handleOutClass = async (req, res) => {
    try {
      ...
      let check = await studentClassService.outClassByStudent({
        classId,
        studentId: userId,
      });
      if (!check) {
  ...
  getInforExamByClassId = async (req, res) => {
    try {
      ...
      if (data.typeId === keyMap.examTypeId.EXAM) {
        dataInvigilator =
          await examInvigilatorService.getInforInvigilatorOfExam({
            examId: data.id,
          });
      }
      let checkIsDone = await resultHistoryService.isDone({
        examId,
        studentId: userId,
      });
      if (!data) {
  ...
  saveResultExamOrAssignmentToHistory = async (req, res) => {
    try {
      ...
      let checkCreate = await resultHistoryService.saveResult({
        examId,
        studentId,
        result,
        time,
  ...
  saveResultChoiseQuestion = async (req, res) => {
    try {
      ...
      let checkCreate = await resultExamService.saveListResultChoiseQuestion({
        dataChoise,
      });
      if (!checkCreate) {
        return res.status(200).json({
  ...
  getListchoiseQuestionOfStudent = async (req, res) => {
    try {
      ...
      let data = await resultExamService.getListchoiseQuestionOfStudent({
        studentId,
        examId,
      });
  ...
  getMissionOfStudent = async (req, res) => {
    try {
      ...
      let listClassIdIsDone =
        await resultHistoryService.getListExamIsDoneOfStudent({ studentId });
      listClassIdIsDone = listClassIdIsDone.map((item) => item.examId);
      let listClassId = await studentClassService.getListClassIdByStudentId({
        studentId,
      });
      listClassId = listClassId.map((item) => item.classId);

      let listExamIsNotDone =
        await examService.getListExamIsNotDoneByListClassId({
          listClassId,
          listClassIdIsDone,
        });
  ...
  postNewNotify = async (req, res) => {
    try {
      ...
      let checkCreate = await notifyService.postNewNotify({
        receiverId,
        senderId: userId,
        contentEN,
        contentVI,
  ...
  searchClassByName = async (req, res) => {
    try {
      ...
      let classIsJoined = await studentClassService.getListClassIdIsJoined({
        studentId: userId,
      });
      let data = await classService.searchClassByName({
        search,
        classIsJoined,
      });
  ...
  deleteOneNotify = async (req, res) => {
    try {
      ...
      let checkDelete = await notifyService.deleteOneNotifyByNotifyId({
        notifyId,
      });
      if (!checkDelete) {
        return res.status(200).json({
  ...
  createNewInvigilator = async (req, res) => {
    try {
      ...
      let checkCreate = await examInvigilatorService.createNewInvigilator({
        examId,
        invigilatorId,
      });
  ...
  getReviewResultOfStudentInExam = async (req, res) => {
    try {
      ...
      let studentOfClassData =
        await studentClassService.getListStudentDataOfClass({ classId });
      let dataPoint =
        await resultHistoryService.getListStudentPointByListExamId({ examId });
  ...
  createRelationshipAccount = async (req, res) => {
    try {
      ...
      let isCheckStudentExist = await relationshipService.checkStudentExist({
        studentId,
      });
      ...
      let checkCreate = await relationshipService.createNewRelationship({
        studentId,
        parentsId,
      });
  ...
  getListDataOfRelationshipAccount = async (req, res) => {
    try {
      ...
      let data = await relationshipService.getListDataOfRelationshipAccount({
        parentsId: userId,
      });
  ...
  deleteOneRelationshipByRelationshipId = async (req, res) => {
    try {
      ...
      let checkDelete =
        await relationshipService.deleteOneRelationshipByRelationshipId({
          relationshipId,
        });
      if (!checkDelete) {
        return res.status(200).json({
  ...
  getListClasOfStudentByParentsId = async (req, res) => {
    try {
      ...
      let data = await relationshipService.getListClassOfStudentByParentsId({
        parentsId,
      });
      ...
      data.forEach((obj) => {
        ...
        if (!mergedMap.has(key)) {
          const mergedObject = {};
          mergedObject.classIdOfStudentData = obj.classIdOfStudentData
            ? [obj.classIdOfStudentData]
            : [];
  ...
  getOneQuestionById = async (req, res) => {
    try {
      const { questionId } = req.query;
      if (!questionId) {
        return res.status(400).json({
          result: false,
          messageEN: "Missing questionId",
          messageVI: "Thiếu mã câu hỏi",
        });
      }
      ...
      if (!question) {
        return res.status(404).json({
          ...
          messageVI: "Không tìm thấy câu hỏi",
        });
      }

      return res.status(200).json({
        result: true,
        messageEN: "Get question detail successfully",
        messageVI: "Lấy chi tiết câu hỏi thành công",
        data: question,
      });
    } catch (error) {
  ...
  getClassOfStudent = async (req, res) => {
    try {
      ...
      if (!studentId) {
        return res.status(400).json({
          result: false,
          messageEN: "Missing studentId",
          messageVI: "Thiếu studentId",
        });
      }

      console.log(studentId, "studentId");

      const classData = await studentClassService.getClassByStudentId({
        studentId,
      });
      ...
      return res.status(200).json({
        result: true,
        messageEN: "Get class successfully",
        messageVI: "Lấy lớp học thành công",
        data: classData,
      });
    } catch (error) {
      return res.status(500).json({
  ...
  openAssignmentByTeacher = async (req, res) => {
    try {
      ...
      const updated = await examService.openExamByExamId({
        examId: assignmentId,
      });
      ...
      if (!updated) {
        return res.status(200).json({
          result: false,
          messageEN: "Open assignment failed",
          messageVI: "Mở bài tập thất bại",
        });
      }

      return res.status(200).json({
        result: true,
        messageEN: "Open assignment successfully",
        messageVI: "Mở bài tập thành công",
      });
    } catch (error) {
      console.log(error);
      return res.status(400).json({
  ...
  getReviewResultOfAssignment = async (req, res) => {
    try {
      ...
      const students = await studentClassService.getListStudentDataOfClass({
        classId,
      });
      const scores = await resultHistoryService.getListStudentPointByListExamId(
        {
          examId: assignmentId,
        }
      );
      ...
      const combined = students.map((student) => {
        const record = scores.find((s) => s.studentId === student.id);
        return {
          id: student.id,
      ...
      return res.status(200).json({
        result: true,
        messageEN: "Get assignment scores successfully",
        messageVI: "Lấy điểm bài tập thành công",
        data: combined,
      });
    } catch (error) {
      console.log(error);
