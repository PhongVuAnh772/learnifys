...
class systemController {
  handleLogin = async (req, res) => {
    try {
      ...
      let token = jwt.sign(
        { userId: checkEmailExists.id, roleId: checkEmailExists.roleId },
  ...
  handleRegisterAccount = async (req, res) => {
    try {
      ...
      let checkCreateAccount = await userService.createNewAccount({
        email,
        password,
        roleId,
      });
  ...
  handleRegisterAccountInformation = async (req, res) => {
    try {
      ...
      let checkUpdateInformations = await userService.UpdateInformation({
        email,
        firstName,
        lastName,
  ...
  handleSendEmailForgotPass = async (req, res) => {
    try {
      ...
      return res.status(200).json({
        ...
        messageVI:
          "Vui lòng kiểm tra email để có thể thực hiện việc lấy lại mật khẩu",
      });
    } catch (error) {
      return res.status(400).json({
        result: false,
  ...
  handleChangePass = async (req, res) => {
    try {
      ...
      let checkVerifyIdentity = await userService.checkVerifyIdentity({
        email,
        phoneNumber,
      });
      if (!checkVerifyIdentity) {
        return res.status(200).json({
          result: false,
          messageEN:
            "Password change failed because user authentication was incorrect",
          messageVI: "Thay mật khẩu thất bại vì xác thực người dùng không đúng",
        });
      }
      let checkChangePass = await userService.changePass({
        email,
        newPassword,
      });
      if (!checkChangePass) {
