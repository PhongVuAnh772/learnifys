...
let sendAttachment = async (dataSend) => {
  ...
  let infor = await transporter.sendMail({
    ...
    to: dataSend.email,
    subject:
      dataSend.language === "en"
        ? "The Result of appointment appointment"
...
let getBodyHTMLRegisterEmail = (dataSend) => {
  ...
  if (dataSend.language === keyMap.VI) {
    result = `
        ...
        <a href="http://127.0.0.1:5000/api/register/extra-infor/${dataSend.receiverEmail}/${dataSend.language}">click here</a>
        <div>Xin chân thành cảm ơn quý khách đã tin tưởng xử dụng dịch vụ của chúng tôi</div>
    `;
  } else {
    result = `
  ...
  try {
    let transporter = nodemailer.createTransport({
      ...
      auth: {
        // user: process.env.EMAIL_APP,
        // pass: process.env.EMAIL_APP_PASSWORD,
        user: "vuanhphong2209@gmail.com",
        pass: "ydthzihkuzkqnhor",
      },
    });

    let infor = await transporter.sendMail({
      from: '"SMART LEARN" <vuanhphong2209@gmail.com>',
      to: dataSend.receiverEmail,
      subject:
        dataSend.language === keyMap.EN
